...
void reset(Resource<GameState> game_state,
  ...
  if (transform.position.z < -2) {
    ...
    camera_transform = Transform(glm::vec3(0, -12, 1.5), glm::quat({glm::half_pi<float>() * 0.9, 0, 0}));
...
void affect_arrow(Resource<input::ButtonInput<input::KeyCode>> keyboard,
  ...
  if (keyboard->is_pressed(input::KeyCode::LeftArrow) && game_state->arrow_angle.z < glm::pi<float>() / 5.) {
    game_state->arrow_angle.z += 0.4 * time->delta_seconds();
  }
  if (keyboard->is_pressed(input::KeyCode::RightArrow) && game_state->arrow_angle.z > -glm::pi<float>() / 5.) {
    game_state->arrow_angle.z -= 0.4 * time->delta_seconds();
  }
  if (keyboard->is_pressed(input::KeyCode::Space) && game_state->arrow_angle.x < glm::pi<float>() / 5.) {
    game_state->arrow_angle.x += 0.4 * time->delta_seconds();
  }
...
void ball_control(Resource<input::ButtonInput<input::MouseButton>> mouse_buttons,
                  ...
                  Resource<GameState> game_state) {
  if (game_state->is_rolling) return;
  if (mouse_buttons->is_just_pressed(input::MouseButton::Left)) {
    auto [_, ball_rigidbody, ball_motion] = ball.single();
    ball_motion.linear += glm::quat(game_state->arrow_angle) * glm::vec3(0., game_state->arrow_power * 15, 0.);
...
void setup(Resource<asset::AssetManager> asset_manager,
           ...
           Resource<RigidBodyWorld> rigidbody_world,
  ...
  cmd.spawn(Ball {}, ball, ball_mat.at(0), Transform(glm::vec3(0, -8.5, 0.0754), glm::quat({0, 0, 0})), RigidBody(1.), Motion({0, 0, 0}), Collider::primitives::Sphere(.0753));
  cmd.spawn(alley, alley_mat.at(0), Transform(glm::vec3(0, 0, 0), glm::quat({0, 0, 0})), RigidBody(INFINITY), Motion(), Collider::primitives::Quad({9.6, 1.}));
  ...
  cmd.spawn(Arrow {}, cube_handle, mat_white, Transform(glm::vec3(0, -8.5, 0.0754), glm::quat({0., 0., 0.}), {0.05, 0.6, 0.05}));
}
