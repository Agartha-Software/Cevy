...
int initial_setup(Resource<asset::AssetManager> asset_manager,
  ...
  cmd.spawn(physics::Spring{anchor.id(), sphere_a.id(), 0.1, 0.2}, Transform(), cube_handle, mat_white);
  cmd.spawn(physics::Spring{anchor.id(), sphere_b.id(), 0.1, 0.2}, Transform(), cube_handle, mat_white);
  return 0;
}

void click_sys(
  Query<Transform, option<TransformVelocity>> transforms, Commands cmd, Query<const Camera, const Transform> camera,
...
void pos_reset(Resource<Time> time,
               Query<Entity, Transform, TransformVelocity> transforms,
               Resource<input::ButtonInput<input::KeyCode>> keyboard,
  ...
  if (keyboard->is_just_pressed(input::KeyCode::R)) {
    for (auto [entity, origin] : origins) {
      ...
      vel.position = origin.velocity.position;
    }
  }
}
