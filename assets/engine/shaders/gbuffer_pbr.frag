...
layout (binding = 1) uniform sampler2D pbr_texture;
...
void main() {
    ...
    vec4 pbr_sample = texture(pbr_texture, texCoord);

    fresnel = mix(fresnel, 0, pbr_sample.r);
    ...
    vec3 specular_final = (1 - fresnel) * specular_const *  mix(vec3(1), diffuse_sample.rgb, pbr_sample.r);
    gSpecular = vec4(specular_final, roughness_const * pbr_sample.a);
