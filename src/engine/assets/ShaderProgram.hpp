...
** C++evy
** file modified from existing
** below is the original comment heading
*/
...
public:
  ...
  ShaderProgram(ShaderProgram&& other) : ShaderProgram() {
    *this = std::move(other);
  }
  ...
  ShaderProgram& operator=(ShaderProgram&& other) {
    ...
    this->shaderCount = other.shaderCount;
    other.shaderCount = 0;
    this->attributeMap = std::move(other.attributeMap);
    other.attributeMap.clear();
    this->uniformMap = std::move(other.uniformMap);
    other.uniformMap.clear();
  ...
  inline void use()
  {
    ...
    {
      std::string msg = (std::stringstream("Shader program ") << programId).str();
