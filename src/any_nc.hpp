...
using namespace std;
  class any_nc
  {
	...
	= enable_if<__and_<is_move_constructible<_Tp>,
  ...
  public:
    ...
    template <typename _Tp, typename... _Args, typename _VTp = decay_t<_Tp>,
      ...
      explicit
      any_nc(in_place_type_t<_Tp>, _Args&&... __args)
      : _M_manager(&_Mgr::_S_manage)
      {
	_Mgr::_S_create(_M_storage, std::forward<_Args>(__args)...);
      }
    ...
    template <typename _Tp, typename _Up, typename... _Args,
	      ...
	      __any_constructible_t<_VTp, initializer_list<_Up>&,
				    _Args&&...> = false>
      explicit
      any_nc(in_place_type_t<_Tp>, initializer_list<_Up> __il, _Args&&... __args)
      : _M_manager(&_Mgr::_S_manage)
      {
	_Mgr::_S_create(_M_storage, __il, std::forward<_Args>(__args)...);
      }
    ...
    // }
        any_nc&
    operator=(const any_nc& __rhs) = delete;
     ...
     */
    any_nc&
    operator=(any_nc&& __rhs) noexcept
    {
      if (!__rhs.has_value())
	reset();
      else if (this != &__rhs)
  ...
  private:
    ...
    template<typename _Tp>
      ...
      {
	static void
	_S_manage(_Op __which, const any_nc* __anyp, _Arg* __arg);
  ...
  template <typename _Tp, typename... _Args>
    inline
    enable_if_t<is_constructible_v<any_nc, in_place_type_t<_Tp>, _Args...>, any_nc>
  ...
  template <typename _Tp, typename _Up, typename... _Args>
    inline
    enable_if_t<is_constructible_v<any_nc, in_place_type_t<_Tp>,
				   initializer_list<_Up>&, _Args...>, any_nc>
  ...
  template<typename _Tp>
    void
    any_nc::_Manager_internal<_Tp>::
    _S_manage(_Op __which, const any_nc* __any, _Arg* __arg)
    {
  ...
  namespace __detail::__variant
  {
    ...
    template<>
      struct _Never_valueless_alt<std::any_nc>
      : std::true_type
      { };
  }  // namespace __detail::__variant
