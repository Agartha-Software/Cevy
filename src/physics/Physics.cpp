 ...
 * @param query
 */
void cevy::physics::RigidBody::system(ecs::Query<Entity, RigidBody, const Collider, engine::Transform, option<engine::TransformVelocity>>
        query) {
...
void cevy::physics::RigidBodyWorld::system(
    Query<RigidBody, engine::TransformVelocity, engine::Transform, Collider> query,
    Resource<Time> time, Resource<RigidBodyWorld> world) {
  if (world->dragDensity != 0) {
    for (auto [body, velocity, transform, collider] : query) {
      if (body.iMass != 0) {

        body.acceleration -=
            velocity.position * glm::length(velocity.position) * world->dragDensity *
            collider.dragCoefficient * collider.area * glm::dot(transform.scale, transform.scale) *
            float(time->delta_seconds()) /* * float(time->delta_seconds())  */* body.iMass / 2.f;
      }
    }
  }
